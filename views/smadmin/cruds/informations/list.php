...
if(isset($_GET["del"]) && is_numeric ($_GET["del"])) {
	...
	$action = db::delete("c_informations", array("id" => $_GET["del"]));
	if($action) {
	send("Successfully Deleted", "success", "crud.php?c=informations");
	}else {
...
if(isset($_POST["tableitems"])) {
	...
	foreach($_POST["tableitems"] as $id) {
		$action = db::delete("c_informations", array("id" => $id));
		$action = true;
		if(!$action) {
			send("The item couldn't deleted.", "error");
		}
...
?>
<a class="btn black left" href="?c=informations&p=new">Add</a>
<?php $datatables=TRUE; ?>
	...
	$(document).ready(function() {
		oTable  = $('#crud').dataTable( {
			...
			"fnServerData": function ( sSource, aoData, fnCallback ) {
						aoData.push( { "name": "c", "value": "c_informations" } );
				$.ajax( {
					"dataType": 'json', 
					"type": "GET", 
					"url": sSource, 
			...
			"aoColumns": [
				...
				{"sClass": "textbox order1",  "sTitle": "Başlık", "sType": "string"},
				{"sClass": "textarea order2", "bVisible": false, "sTitle": "Açıklama", "bSearchable": false, "sType": "html"},
				{"sClass": "option order3", "bVisible": false, "sTitle": "Galeri", "sType": "string"},
				...
				{"mDataProp": null, "sClass": "control center", "fnRender": function (events) {return '<a href="?c=informations&p=edit&id='+events.aData[0]+'&createnew"><span class="ui-icon ui-icon-newwin"></span></a><a href="?c=informations&p=edit&id='+events.aData[0]+'&viewonly"><span class="ui-icon ui-icon-search"></span></a><a href="?c=informations&p=edit&id='+events.aData[0]+'"><span class="ui-icon ui-icon-pencil"></span></a><a confirm="You have selected 1 entry for permanent deletion. This action cannot be undone." href="?c=informations&del='+events.aData[0]+'"><span class="ui-icon ui-icon-trash"></span></a>';}, "bSortable": false}
			]
		} );
